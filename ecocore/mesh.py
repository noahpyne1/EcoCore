...
from ecocore.scripts.generate_normals import generate_normals
from ecocore.scripts.project_uvs import project_uvs
from ecocore.scripts.colorize import colorize
...
class Mesh(NodePath):
    ...
    def save(self, name='', path=application.compressed_models_folder):
        ...
        if name.endswith('ecocoremesh'):
            with open(path / name, 'w') as f:
                recipe = self.recipe.replace('LVector3f', '')
                f.write(recipe)
            print('saved .ecocoremesh to:', path / name)

        elif name.endswith('.obj'):
            from ecocore.mesh_importer import ecocore_mesh_to_obj
            ecocore_mesh_to_obj(self, name, path)
