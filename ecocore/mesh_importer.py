...
def load_model(name, path=application.asset_folder):
    ...
    for filetype in ('.bam', '.ecocoremesh', '.obj', '.blend'):
        ...
        for filename in path.glob(f'**/{name}{filetype}'):
            ...
            if filetype == '.ecocoremesh':
                try:
                    ...
                    print('invalid ecocoremesh file:', filename)

            if filetype == '.obj':
                print('found obj', filename)
                m = obj_to_ecocoremesh(path=path, name=name, save_to_file=False)
                m = eval(m)
                m.path = filename
                m.name = name
                imported_meshes[name] = m
...
def obj_to_ecocoremesh(
    ...
    for f in path.glob(f'**/{name}.obj'):
        ...
        outfilepath = outpath / (os.path.splitext(f)[0] + '.ecocoremesh')
        ...
        print('saved ecocoremesh to:', outfilepath)
    ...
    for f in files:
        if f.endswith('.blend'):
            ...
            for o in blend.list('Object'):
                ...
                file_name = ''.join([f.split('.')[0], '.ecocoremesh'])
                if number_of_objects > 1:
                    file_name = ''.join([f.split('.')[0], '_', object_name, '.ecocoremesh'])
...
def ecocore_mesh_to_obj(mesh, name='', out_path=application.compressed_models_folder, max_decimals=3):
    from ecocore.string_utilities import camel_to_snake
...
if __name__ == '__main__':
    ...
    # ecocore_mesh_to_obj(e.model, name='quad_export_test')
