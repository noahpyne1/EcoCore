...
if len(sys.argv) > 1:
    if sys.argv[1] == 'help' or sys.argv[1] == '--help':
        print(
            '''package ecocore application for windows10.
...
# def copy_ecocore():
print('copying ecocore')
import importlib
spec = importlib.util.find_spec('ecocore')
ecocore_path = Path(spec.origin).parent
dest = build_folder / 'python/Lib/site-packages/ecocore'
dest.mkdir(parents=True, exist_ok=True)
copytree(ecocore_path, dest, ignore=shutil.ignore_patterns('samples', 'unused'))
...
import subprocess
subprocess.call([
    f'{ecocore_path}\\scripts\\_bat_to_exe.bat',
